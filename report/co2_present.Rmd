...
Our polynomial model looks like it did a pretty good job at predicting $CO_2$ up to 2020. It is missing the peaks and valleys, but it looks to capture the average yearly increase in $CO_2$ levels.

```{r linear-compare}
co2_poly_preditions <- read_csv("./data/co2_poly_preditions.csv") |> 
  mutate(index = yearmonth(index))

co2_poly_preditions |> as_tsibble()

lm.plot <- ggplot(co2_present, aes(x = index)) +
  geom_line(aes(y = average, colour = "Actual")) +
  geom_line(data = co2_poly_preditions, aes(x = index, y = predictions, colour = "Predicted")) +
  labs(title = "CO2 Levels with Fitted Linear Trend", x = "Year", y = "CO2 (ppm)") +
  theme_minimal()
```

```{r lm-plots, include=TRUE,fig.cap="Comparing predicted Atmospheric CO2 Levels with Actual level - Polynomial Model", fig.width=9, fig.height=3}
lm.plot
```
...
In the 1997 report, we used an ARIMA ARIMA(3,1,0)(2,1,0)[12] model to forecast atmospheric $CO_2$ levels. Both the ARIMA model forecast and the the realized $CO_2$ show an upward trend with seasonal pattern. However, the model predicted lower values than the realized values in the log term. 

```{r ARIMA-compare}
co2.Arima.predictions <- read_csv("./data/co2_arima_preditions.csv") |> 
  mutate(index = yearmonth(index))

co2.Arima.predictions |> as_tsibble()

arima.plot <- ggplot(co2_present, aes(x = index)) +
  geom_line(aes(y = average, colour = "Actual")) +
  geom_line(data = co2.Arima.predictions, aes(x = index, y = value, colour = "Predicted - Log")) +
  labs(title = "CO2 Levels with Fitted ARIMA Trend", x = "Year", y = "CO2 (ppm)") +
  theme_minimal()
```

```{r arim-plot, include=TRUE,fig.cap="Comparing predicted Atmospheric CO2 Levels with Actual level - Polynomial Model", fig.width=9, fig.height=3}
arima.plot
```
...
In 1997, we predicted that CO2 levels would reach 420 PPM by March 2025, about a year early. Actual CO2 levels reached 420 PPM by March of 2022.
