...
In this section we will explore new models and train them on the present data then evaluate their performance to see if we can estimate a better model that what we estimated in the 1997 report.
...
We made two copies of the aggregated `NOAA` data and seasonally adjusted one of them STL decomposition. After that, we split both datasets (the seasonally adjusted $(SA)$ and the non-seasonally adjusted $(NSA)$) into training and test sets, using the last two years of observations as the test sets.
```{r split new data}
...
Based on the EDA performed earlier, it was challenging to estimate the $p$ and $q$ terms for the ARIMA model just by looking at the `ACF` and `PACF` plots. We estimated two non-stepwise ARIMA models for each dataset, one with log transformation and one without. Using the information criteria `AICc` in Table \@ref(tab:sa-arima-results), we see that the best $SA$ model was an ARIMA(1,1,1)(2,0,0)[12] with drift and a log transformation. This model has one `AR` term, one `MA` term, and first-order differencing term. Additionally, It has two seasonal `AR` term with a period of 12 months. The best $NSA$ model was an ARIMA(1,1,2)(0,1,1)[12] with a log tranformation. This model has one `AR` term, two `MA` term, and first-order differencing term. It also has one seasonal `MA` term and one seasonal differencing term with a period of 12 months. We selected both of these models because they had the lowest `AICc`. We will examine the residuals to see if they resemble white noise.
...
```{r sa-poly-plot, include=TRUE, fig.cap="SA trained Ploynomial Model - In-Sample vs Out-of-Sample", fig.width=9, fig.height=3}
...
In 1997, we predicted that CO2 levels would reach 500 ppm by the 2050 May. Our updated prediction is that we will reach 500 ppm by `r round(pred_500ppm$index[1], 2)`. This indicates that the updated data may support a slowing of the growth in atmospheric CO2. By 2122, our model predicts that CO2 levels will reach `r round(pred_2121$.mean, 2)` ppm. As with the 1997 report, we have low confidence in these predictions, as we are well beyond the range of our data, and have no way of accounting for improvements in efficiency, grid electrification, etc.

# Conclusion

The Polynomial model estimated in 1997 report accurately predicted the realized atmospheric $CO_2$ levels and we failed to reject the null hypothesis. However, the ARIMA model had a significant difference between the predicted and actual  atmospheric $CO_2$ levels and we had to reject the null hypothesis.

The new models trained using seasonally adjusted data were outperformed by those trained using non-seasonally adjusted data.

The updated atmospheric CO2 data shows that the increasing trend in CO2 levels continued roughly as expected. We still see significant coefficients in our estimted predictive models using present data, and forecast that CO2 levels will continue to rise into the future, barring any significant intervention. As follows, we again reject our null hypothesis.
