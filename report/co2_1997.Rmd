...
The `ACF` decays slowly but does not dampen below the significance level even after lag 24. This suggests a strong autocorrelation in the $CO_2$ values while the `PACF` drops shortly after lag 1 but still have oscillating pattern with few lags above the significance level. This suggests that this series may have a unit root. Both `ACF` and `PACF` show seasonality patterns.
...
To estimate the $i$ parameter, we used the `KPSS` test to check for stationarity. Initially, the p-value was 0.01, leading us to reject the null hypothesis, indicating non-stationarity. After differencing once, the p-value wend up to 0.1, and we failed to reject the null hypothesis, indicating that the data is stationary after one differencing.
...
Using our best model the RIMA(3,1,0)(2,1,0)[12], we predicted that the $CO_2$ levels will reach 420 ppm by `r date_420` with `r round(cof.int.420,0)`% confidence interval while it predicts it will reach 500 ppm by `r date_500` with `r round(cof.int.500,0)`% confidence interval.

By 2100, our model predicts that CO2 levels will be at about `r avg.co2.2100` ppm on average. This prediction is likely highly inaccurate, as we are predicting out much further than our training data set. Additionally, this model does not factor in any events that might impact the rate of increase. For example, increases in efficiency and decarbonization of the grid may decrease the slope of the graph.
