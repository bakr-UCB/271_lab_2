...
co2.ts %>% features(value, unitroot_ndiffs)
...
We estimated an ARIMA(2,1,0)(2,1,0)[12] model as a baseline plus four additional models, using both the regular and the logarithmic transformed $CO_2$ value once using stepwise and and non-stepwise search.
...
forecasts <- co2_fit |> 
  select(arima_fit_log.search) |>
  forecast(h="103 years")

co2.Arima.predictions <- forecasts |>
  as_tsibble() |> 
  mutate(value = .mean) |>
  select(index, value) |>
  filter(index <= yearmonth("2024 Jun"))
