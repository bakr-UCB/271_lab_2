...
log.lm.mod.plot <- co2.ts |> 
  ...
  geom_line() +
  geom_line(aes(y = predict(log.mod.linear)), color = "cornflowerblue") +
  labs(title = "CO2 Levels with Fitted Linear Time Trend Model", x = "Month", y = "CO2 (ppm)")
...
season_dummies <- model.matrix(~ season - 1, data = co2.ts)

co2.ts <- bind_cols(co2.ts, season_dummies)

mod.quadratic.season <- lm(value ~ as.numeric(index) + I(as.numeric(index)^2) + factor(season), data = co2.ts) # not sure this is correct
...
models <- list(mod.linear, log.mod.linear, mod.quadratic)

# Add appropriate model names
names(models) <- c("Linear Model", "Log Linear Model", "Quadratic Model")
```
