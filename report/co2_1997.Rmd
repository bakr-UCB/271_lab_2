...
The residuals line and `ACF` plots Figure \@ref(fig:ARIMA-residual) from the ARIMA(3,1,0)(2,1,0)[12] model shows that all autocorrelations are within the threshold limits, indicating that the residuals are behaving like white noise which is confirmed by `Ljung Box` test with a p-value = 0.358 we fail to reject the null hypothesis $H_0: \text{data is independently distributed}$.
